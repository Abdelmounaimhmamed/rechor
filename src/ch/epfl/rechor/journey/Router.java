...
public record Router(TimeTable timeTable) {
    ...
    private List<Long> criteriaList(ParetoFront.Builder builder) {
        List<Long> list = new ArrayList<>();
        builder.forEach(list::add);
        return list;
    }
    ...
    public Profile profile(LocalDate date, int arrStationId) {
        ...
        for (int connId = 0; connId < numConnections; connId++) {
            ...
            for (long crit : criteriaList(stationBuilder)) {
            ...
            for (int s : reachableStations) {
                ...
                for (long crit : criteriaList(f)) {
                    long newCrit = PackedCriteria.pack(
                            connections.depMins(connId) - walkingDuration,
